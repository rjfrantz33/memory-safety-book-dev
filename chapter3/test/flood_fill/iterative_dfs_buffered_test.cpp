...
TEST(FloodFillIterativeDFSBuffered, CompletesFullGrid) {
    ...
    auto status =
        ch3::floodFillIterativeDFS(grid, 0, 0, ch3::kFirstClusterId, buffer);
...
TEST(FloodFillIterativeDFSBuffered, OutOfBoundsStartReturnsCompleted) {
    ...
    ch3::WorklistBuffer buffer{};
    auto status =
        ch3::floodFillIterativeDFS(grid, 10, 10, ch3::kFirstClusterId, buffer);
    EXPECT_EQ(status, ch3::FloodFillStatus::kCompleted);
}
...
TEST(FloodFillIterativeDFSBuffered, MatchesFunctionLocalVariant) {
    ...
    auto s_local =
        ch3::floodFillIterativeDFS(grid_local, 0, 0, ch3::kFirstClusterId);
    auto s_buffered = ch3::floodFillIterativeDFS(
