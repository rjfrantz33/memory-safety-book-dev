...
TEST(FloodFillIterativeBFSBuffered, CompletesFullGrid) {
    ...
    auto status = ch3::floodFillIterativeBFS(grid, 0, 0, ch3::kFirstClusterId, buffer);
...
TEST(FloodFillIterativeBFSBuffered, OutOfBoundsStartReturnsCompleted) {
    ...
    ch3::WorklistBuffer buffer{};
    auto status = ch3::floodFillIterativeBFS(grid, -1, 0, ch3::kFirstClusterId, buffer);
    EXPECT_EQ(status, ch3::FloodFillStatus::kCompleted);
}
...
TEST(FloodFillIterativeBFSBuffered, MatchesFunctionLocalVariant) {
    ...
    auto s_local    = ch3::floodFillIterativeBFS(grid_local,    0, 0, ch3::kFirstClusterId);
    auto s_buffered = ch3::floodFillIterativeBFS(grid_buffered, 0, 0, ch3::kFirstClusterId, buffer);
